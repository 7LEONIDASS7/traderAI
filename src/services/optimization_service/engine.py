...
class OptimizationEngine:
    ...
    def _get_prompt_performance_data(self, prompt_name: str, days_history: int) -> Dict[str, Any]:
        ...
        Uses the memory indexing system for efficient aggregation.
        """
        log.debug(f"Querying performance data for prompt '{prompt_name}' over last {days_history} days.")
        
        try:
            # Get real performance metrics from memory storage
            metrics = self.memory.get_performance_metrics(prompt_name, days_history)
            
            # Convert to expected format for optimization template
            performance_data = {
                "prompt_version": "current",  # Could be enhanced to track versions
                "signals_generated": metrics.get("signals_generated", 0),
                "trades_executed": metrics.get("trades_executed", 0),
                "win_rate": metrics.get("win_rate", 0.0),
                "avg_pl": metrics.get("avg_profit_loss", 0.0),
                "currency": "USD",
                "error_summary": metrics.get("error_types", {}),
                ...
                "data_quality": "real"  # Flag to indicate this is real data
            }
            
            # Add example rationales if we have trade data
            if metrics.get("trades_completed", 0) > 0:
                performance_data["example_success_rationale"] = f"System generated {metrics['signals_generated']} signals with {metrics['win_rate']:.1f}% win rate"
                if metrics.get("win_rate", 0) < 50:
                    performance_data["example_fail_rationale"] = f"Low win rate ({metrics['win_rate']:.1f}%) suggests prompt may need refinement for market conditions"
                else:
                    performance_data["example_fail_rationale"] = f"Some losses occurred but overall performance is positive"
            else:
                performance_data["example_success_rationale"] = "No completed trades yet to analyze"
                performance_data["example_fail_rationale"] = "Insufficient trade history for analysis"
            
            # Log summary of real data retrieved
            log.info(f"Retrieved real performance data for '{prompt_name}': {performance_data['signals_generated']} signals, "
                    f"{performance_data['trades_completed']} completed trades, "
                    f"{performance_data['win_rate']:.1f}% win rate, "
                    f"${performance_data['total_profit_loss']:.2f} total P/L")
            
            return performance_data
            
        except Exception as e:
            log.error(f"Error retrieving performance data for prompt '{prompt_name}': {e}", exc_info=True)
            
            # Fallback to minimal data structure on error
            return {
                ...
                "example_success_rationale": "Unable to retrieve performance data",
                "example_fail_rationale": f"Error occurred: {str(e)}",
                "data_quality": "error",
                "error": str(e)
            }
